...
class Quiz:
	...
	def buttons(self):
		...
		# next Question
		next_button = ttk.Button(gui, text="Next",command=self.next_btn,
		width=10,)
		...
		quit_button = ttk.Button(gui, text="Sair", command=gui.destroy,
		width=5,) 
	...
	def display_title(self):
		...
		title = Label(gui, text="@SETPROGRAMACAO QUIZ",
		width=50, height=2, bg="green",fg="white", font=("ariel", 20, "bold"))
	...
	def radio_buttons(self):
		...
		first_opt_position = 150
		
		# adding the options to the list
		while len(options_list) < 4:
			
			# setting the radio button properties
			radio_btn = ttk.Radiobutton(gui,text=" ",variable=self.opt_selected,
			value = len(options_list)+1,)
			
			# adding the button to the list
			options_list.append(radio_btn)
			
			# placing the button
			radio_btn.place(x = 100, y = first_opt_position)
			
			# incrementing the y-axis position by 40
			first_opt_position += 40
...
style = ttk.Style(gui)
gui.tk.call("source", "forest-light.tcl")
gui.tk.call("source", "forest-dark.tcl")
style.theme_use("forest-dark")
# set the title of the Window
gui.title("@setprogramacao Quiz")
